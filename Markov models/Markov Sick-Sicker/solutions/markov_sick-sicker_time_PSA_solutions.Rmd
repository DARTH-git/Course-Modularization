...
a_P_notrt <- array(0,                                    # Create 3-D array
                   dim      = c(n_s, n_s, n_t),
                   dimnames = list(v_n, v_n, 0:(n_t-1))) # name dimensions of the array
```
...
a_P_notrt["H", "S1", ]  <- p_HS1
a_P_notrt["H", "D", ]   <- p_HD
# from Sick
a_P_notrt["S1", "H", ]  <- p_S1H
a_P_notrt["S1", "S1", ] <- 1 - (p_S1H + p_S1S2 + p_S1D)
a_P_notrt["S1", "S2", ] <- p_S1S2
...
valid <- apply(a_P_notrt, 3, function(x) sum(rowSums(x))==n_s)
...
v_os_notrt_td <- 1 - m_M_notrt[, "D"]         
# alternative way of calculating the OS probability   
v_os_notrt_td <- rowSums(m_M_notrt[, 1:3])    
# create a simple plot showing the OS
plot(age:max_age, v_os_notrt_td, type = 'l', 
     ylim = c(0, 1),
     ylab = "Survival probability",
     xlab = "Age",
     main = "Overall Survival Age-Dependent")   
# add grid
...
```{r}
v_le_td <- sum(v_os_notrt_td) # summing probablity of OS over time (i_e_ life expectancy)
```

## 06.3 Disease prevalence

```{r}
v_prev_td <- rowSums(m_M_notrt[, c("S1", "S2")])/v_os_notrt_td
plot(v_prev_td,
     ylim = c(0, 1),
     ylab = "Prevalence",
     xlab = "Cycle",
...
```{r}
v_ratio_S1S2_td <- m_M_notrt[, "S1"] / m_M_notrt[, "S2"]
plot(0:n_t, v_ratio_S1S2_td,
     xlab = "Cycle", 
     ylab = "Ratio S1 vs S2", 
...
```{r}
plot(df_cea, effect_units = "Quality of Life", xlim=c(16.8,17.8))
```
...
v_names_params <- c('p_HS1', 'p_S1H', 'p_S1S2', 'hr_S1', 'hr_S2', 'c_H', 'c_S1', 'c_S2', 'c_trt', 'c_D', 'u_H', 'u_S1',  
...
```{r}
source(here("functions", "Functions_markov_sick-sicker_time.R"))
# Test function
...
```{r, warning=F}
params_owsa      <- c("p_S1S2", "c_trt", "u_S1", "u_trt") # parameters of interest for OWSA
...
params_all_owsa$min[params_all_owsa$pars %in% params_owsa] <- c(0.05 ,  6000 , 0.65, 0.80)  # min parameter values
params_all_owsa$max[params_all_owsa$pars %in% params_owsa] <- c(0.155, 18000 , 0.85, 0.98)  # max parameter values
...
save(df_psa_input, df_c, df_e, v_names_str, n_str,
     l_psa,
     file = here("output", "markov_sick-sicker_time_PSA_dataset.RData"))
```

## 09.3 Create probabilistic analysis graphs

```{r}
load(file = here("output", "markov_sick-sicker_time_PSA_dataset.RData"))
```
...
save(df_cea_psa, 
     file = here("tables", 
                 "markov_sick-sicker_time_probabilistic_CEA_results.RData"))
# As .csv
write.csv(df_cea_psa, 
          file = here("tables", 
                      "markov_sick-sicker_time_probabilistic_CEA_results.csv"))
