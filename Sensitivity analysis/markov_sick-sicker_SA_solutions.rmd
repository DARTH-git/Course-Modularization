...
gen_psa <- function(n_sim = 1000, seed = 071818){
  ...
  df_psa <- data.frame(
    # Transition probabilities (per cycle)
    p_HS1   = rbeta(n_sim, alpha = 30, beta = 170),     # probability to become sick when healthy
    p_S1H   = rbeta(n_sim, alpha = 60, beta = 60) ,     # probability to become healthy when sick
    p_S1S2  = rbeta(n_sim, alpha = 84, beta = 716),     # probability to become sicker when sick
    p_HD    = rbeta(n_sim, alpha = 10, beta = 1990),    # probability to die when healthy
