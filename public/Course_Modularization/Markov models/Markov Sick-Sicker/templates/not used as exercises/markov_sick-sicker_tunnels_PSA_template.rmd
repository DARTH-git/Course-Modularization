...
v_names_states_td <- c("H", Sick_tunnel, "S2", "D")  # state names
n_states_td       <- length(v_names_states_td)       # number of states
...
m_M_notrt <- m_M_trt <- matrix(NA, 
                                nrow = n_t + 1, ncol = n_states_td,
                                dimnames = list(paste("cycle", 0:n_t, sep = " "), v_names_states_td)) 
...
a_P_notrt <- array(0,                                          # Create 3-D array
                   dim      = c(n_states_td, n_states_td, n_t),
                   dimnames = list(v_names_states_td, v_names_states_td, 0:(n_t-1))) # name dimensions
```
