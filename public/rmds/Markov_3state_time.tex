...
\NormalTok{p_HD <-}\StringTok{ }\KeywordTok{seq}\NormalTok{(}\FloatTok{0.003}\NormalTok{, }\FloatTok{0.01}\NormalTok{, }\DataTypeTok{length.out =}\NormalTok{ n_t)  }\CommentTok{# probabilities to die when sick (age-dependent) - this is now a sequence of numbers, officially v_HD}
...
\NormalTok{a_P[}\StringTok{"Healthy"}\NormalTok{, }\StringTok{"Sick"}\NormalTok{, ]    <-}\StringTok{ }\NormalTok{p_HS}
...
\CommentTok{# Check if transition probabilities are valid (i.e., in [0, 1])}
\NormalTok{m_indices_notvalid <-}\StringTok{ }\KeywordTok{arrayInd}\NormalTok{(}\KeywordTok{which}\NormalTok{(a_P }\OperatorTok{<}\StringTok{ }\DecValTok{0} \OperatorTok{|}\StringTok{ }\NormalTok{a_P }\OperatorTok{>}\StringTok{ }\DecValTok{1}\NormalTok{), }
                               \KeywordTok{dim}\NormalTok{(a_P))}
\KeywordTok{try}\NormalTok{(}
  \ControlFlowTok{if}\NormalTok{(}\KeywordTok{dim}\NormalTok{(m_indices_notvalid)[}\DecValTok{1}\NormalTok{] }\OperatorTok{!=}\StringTok{ }\DecValTok{0}\NormalTok{)\{}
\NormalTok{    v_rows_notval   <-}\StringTok{ }\KeywordTok{rownames}\NormalTok{(a_P)[m_indices_notvalid[, }\DecValTok{1}\NormalTok{]]}
\NormalTok{    v_cols_notval   <-}\StringTok{ }\KeywordTok{colnames}\NormalTok{(a_P)[m_indices_notvalid[, }\DecValTok{2}\NormalTok{]]}
\NormalTok{    v_cycles_notval <-}\StringTok{ }\KeywordTok{dimnames}\NormalTok{(a_P)[[}\DecValTok{3}\NormalTok{]][m_indices_notvalid[, }\DecValTok{3}\NormalTok{]]}
    
\NormalTok{    df_notvalid <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\StringTok{`}\DataTypeTok{Transition probabilities not valid:}\StringTok{`}\NormalTok{ =}\StringTok{ }
\StringTok{                                }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{paste0}\NormalTok{(}\KeywordTok{paste}\NormalTok{(v_rows_notval, v_cols_notval, }
                                                    \DataTypeTok{sep =} \StringTok{"->"}\NormalTok{),}
                                              \StringTok{"; at cycle "}\NormalTok{,}
\NormalTok{                                              v_cycles_notval), }\DataTypeTok{ncol =} \DecValTok{1}\NormalTok{), }
                              \DataTypeTok{check.names =} \OtherTok{FALSE}\NormalTok{)}
    \ControlFlowTok{if}\NormalTok{(verbose)\{}
      \KeywordTok{message}\NormalTok{(}\StringTok{"Not valid transition probabilities"}\NormalTok{)}
      \KeywordTok{stop}\NormalTok{(}\KeywordTok{print}\NormalTok{(df_notvalid), }\DataTypeTok{call. =} \OtherTok{FALSE}\NormalTok{)}
\NormalTok{    \} }
\NormalTok{  \}}
\NormalTok{)}

\CommentTok{# Check if transition probability array is valid}
\NormalTok{valid <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(a_P, }\DecValTok{3}\NormalTok{, }\ControlFlowTok{function}\NormalTok{(x) }\KeywordTok{all.equal}\NormalTok{(}\KeywordTok{sum}\NormalTok{(}\KeywordTok{rowSums}\NormalTok{(x)), n_states))}
...
## 2 0.8966966 0.0923500 0.01095336
## 3 0.8489589 0.1279498 0.02309122
## 4 0.8036620 0.1576028 0.03873525
## 5 0.7606865 0.1820256 0.05728792
\end{verbatim}
...
\includegraphics{Markov_3state_time_files/figure-latex/unnamed-chunk-10-1.pdf}
...
\includegraphics{Markov_3state_time_files/figure-latex/unnamed-chunk-11-1.pdf}
...
\includegraphics{Markov_3state_time_files/figure-latex/unnamed-chunk-13-1.pdf}
