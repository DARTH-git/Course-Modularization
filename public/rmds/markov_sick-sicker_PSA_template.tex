...
\hypertarget{deterministic-sensitivity-analysis---start-here}{%
\section{08 Deterministic Sensitivity Analysis - START
HERE}\label{deterministic-sensitivity-analysis---start-here}}
...
\NormalTok{df_params_owsa <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{pars =} \KeywordTok{c}\NormalTok{( ,  ,  , ), }\CommentTok{# SPECIFY PARAMETERS AND MIN MAX VALUES}
                             \DataTypeTok{min  =} \KeywordTok{c}\NormalTok{( ,  ,  , ), }\CommentTok{# min parameter values}
                             \DataTypeTok{max  =} \KeywordTok{c}\NormalTok{( ,  ,  , )  }\CommentTok{# max parameter values}
\NormalTok{                             )}
...
\CommentTok{# dataframe containing all parameters, their basecase values, and the min and }
\CommentTok{# max values of the parameters of interest}
\NormalTok{df_params_twsa <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{pars =} \KeywordTok{c}\NormalTok{( , ), }\CommentTok{# SPECIFY PARAMETERS AND MIN MAX VALUES}
                             \DataTypeTok{min  =} \KeywordTok{c}\NormalTok{( , ), }\CommentTok{# min parameter values}
                             \DataTypeTok{max  =} \KeywordTok{c}\NormalTok{( , )  }\CommentTok{# max parameter values}
\NormalTok{                             )}
...
\NormalTok{  df_psa <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
    \CommentTok{# Transition probabilities (per cycle)}
    \DataTypeTok{p_HS1   =} \CommentTok{# SPECIFY DISTRIBUTION ,  # probability to become sick when healthy}
    \DataTypeTok{p_S1H   =} \CommentTok{# SPECIFY DISTRIBUTION ,  # probability to become healthy when sick}
    \DataTypeTok{p_S1S2  =} \CommentTok{# SPECIFY DISTRIBUTION ,  # probability to become sicker when sick}
    \DataTypeTok{p_HD    =} \CommentTok{# SPECIFY DISTRIBUTION ,  # probability to die when healthy}
    \DataTypeTok{hr_S1   =} \CommentTok{# SPECIFY DISTRIBUTION ,  # rate ratio of death in S1 vs healthy}
    \DataTypeTok{hr_S2   =} \CommentTok{# SPECIFY DISTRIBUTION ,  # rate ratio of death in S2 vs healthy }
    
    \CommentTok{# State rewards}
    \CommentTok{# Costs}
    \DataTypeTok{c_H   =} \CommentTok{# SPECIFY DISTRIBUTION    , # cost of remaining one cycle in state H}
    \DataTypeTok{c_S1  =} \CommentTok{# SPECIFY DISTRIBUTION    , # cost of remaining one cycle in state S1}
    \DataTypeTok{c_S2  =} \CommentTok{# SPECIFY DISTRIBUTION    , # cost of remaining one cycle in state S2}
    \DataTypeTok{c_Trt =} \CommentTok{# SPECIFY DISTRIBUTION    , # cost of treatment (per cycle)}
    \DataTypeTok{c_D   =} \CommentTok{# SPECIFY DISTRIBUTION    , # cost of being in the death state}
    
    \CommentTok{# Utilities}
    \DataTypeTok{u_H   =} \CommentTok{# SPECIFY DISTRIBUTION  , # utility when healthy}
    \DataTypeTok{u_S1  =} \CommentTok{# SPECIFY DISTRIBUTION  , # utility when sick}
    \DataTypeTok{u_S2  =} \CommentTok{# SPECIFY DISTRIBUTION  , # utility when sicker}
    \DataTypeTok{u_D   =} \CommentTok{# SPECIFY DISTRIBUTION  , # utility when dead}
    \DataTypeTok{u_Trt =} \CommentTok{# SPECIFY DISTRIBUTION) , # utility when being treated}
    \DataTypeTok{d_e   =} \CommentTok{# SPECIFY DISTRIBUTION  , # discount factor for effectiveness}
    \DataTypeTok{d_c   =} \CommentTok{# SPECIFY DISTRIBUTION    # discount factor for costs}
\NormalTok{  )}
