...
p_load( "dplyr", "devtools", "scales", "ellipse", "ggplot2", "lazyeval", "igraph" , "ggraph", "reshape2", "knitr", "stringr", "rstudioapi")                                               
...
p_biopsy_death <- 0.05  # probability of death due to biopsy
...
```{r}
### alternatively using OpenTree
# draw the tree using OpenTree


open_tree(file_name = "HVEtemplate", dir_name = getwd())

# extract the probability weights and outcomes  
df_tree <- evaluate_model("HVEtemplate", n_payoffs = 2)

  total_qaly_no_tx <- df_tree[['Do not treat']]$prob %*% df_tree[['Do not treat']]$payoff1 
  total_cost_no_tx <- df_tree[['Do not treat']]$prob %*% df_tree[['Do not treat']]$payoff2
