...
```{r}
create_tree(file_name = "TestTree", dir_name = getwd())
open_tree(file_name = "CADTree", dir_name = getwd())
```
...
```{r}
v_names_str    <- c("medicine", "surgery")  # names of strategies
n_str          <- length(v_names_str)                   # number of strategies
wtp            <- 100000                                # willingness to pay threshold

# Probabilities
p_death_CAD_medicine          <- 0.3  
p_death_CAD_Surgery           <- 0.2   
p_death_PR_Surgery            <- 0.05   

#Life Expectancy

le_death_CAD_medicine         <- 5
le_death_CAD_Surgery          <- 5
le_death_other_medicine       <- 10
le_death_other_Surgery        <- 10
le_death_PR_Surgery           <- 0
...
df_tree <- evaluate_model("CADTree", n_payoffs = 1)
```
