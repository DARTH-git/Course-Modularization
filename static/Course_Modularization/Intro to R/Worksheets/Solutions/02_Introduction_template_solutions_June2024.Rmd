...
3. Types of data in R (Creating vectors and matrices)
...
**EXERCISE 1** Type `2 + 3` in the *code chunk* below and run. Remember that the code chunk can be run using the green arrow at the right hand corner of the chunk or using the keyboard shortcut CTRL + SHIFT + ENTER. If you only want to run one line of code where your cursor is, you can type CTRL + ENTER.
...
**EXERCISE 8** Run the following *code chunk* to change the last value of `vector1`:
...
The top right hand window will show the contents of `vector1`. Note that if you overwrite the contents of an object, you cannot retrieve the previous contents of the object unless you rerun the initial code.

You can check the contents of an object by typing the name of an object into a *code chunk* and running that line of code. This prints the object in the console and in the R Markdown so you can check it is defined correctly. You can also check the contents in the top right-hand window that displays you *workspace*.
...
**EXERCISE 9** Use `vector1`, `vector2` and `matrixA` to investigate how these different functions are used for vectors and matrices. You can use the *code chunk* below:
...
**EXERCISE 10** Open up and read the help document for the function `sum()`.
...
**EXERCISE 11** Install `R` package *dplyr*. Usually, the command to install a package is run from the console as running it from *code chunks* can cause problems.
