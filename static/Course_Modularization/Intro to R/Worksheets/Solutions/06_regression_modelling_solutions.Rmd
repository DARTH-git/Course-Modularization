...
Before we begin this session, we need to load our modified Framingham dataset into `R` and load required packages. 

```{r, warning=F, message=F}
data1 <- read.csv('framingham1.csv', header = TRUE)
...
library(dplyr)
data2 <- data1 %>%
              select(PREVSTRK, AGE, BMI, CURSMOKE, HEARTRTE, SYSBP, LDLC) 
data2 <- na.omit(data2)
```
...
```{r}
# Your turn
step(logit_model3, direction = "both")
```
...
data1$CURSMOKE <- as.character(data1$CURSMOKE)
data1$CURSMOKE[(is.na(data1$CURSMOKE) == FALSE) & (data1$CURSMOKE == 'yes')] <- 1 
data1$CURSMOKE[(is.na(data1$CURSMOKE) == FALSE) & (data1$CURSMOKE != 1)] <- 0
data1$CURSMOKE <- as.factor(data1$CURSMOKE)
# select only the variables we need and exclude any observation with missing values
data3 <- data1 %>% select(CURSMOKE, AGE, BMI, HEARTRTE, SYSBP, LDLC)
data3 <- na.omit(data3)
# fit model
logit_model4 <- glm(CURSMOKE ~ AGE + BMI + HEARTRTE + SYSBP + LDLC, 
                    family = 'binomial', 
                    data = data3)
...
# Hosmer-Lemeshow test
hoslem.test(data3$CURSMOKE, fitted(logit_model4), g = 10)
# ROC and AUC
# fit the model
roc4 <- roc(data3$CURSMOKE, logit_model4$fitted.values)   
# plot the ROC curve
plot(roc4, col = "red", legacy.axes = TRUE)
# calculate the AUC
auc(roc4)                                                
```
...
step(logit_model4, direction = 'both')
```
