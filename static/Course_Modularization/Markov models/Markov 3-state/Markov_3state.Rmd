...
v_names_states  <- c("H", "S", "D")              # state names, Healthy (H), Sick (S1), Dead (D)
...
p_SD      <- 0.10  # probability of dying when sick    
...
### Fill in matrix 
# from Healthy
m_P_SoC["H", "H"] <- (1 - p_HD) * (1 - p_HS_SoC)
m_P_SoC["H", "S"] <- (1 - p_HD) *      p_HS_SoC
m_P_SoC["H", "D"] <-      p_HD
# from Sick
m_P_SoC["S", "S"] <- 1 - p_SD
m_P_SoC["S", "D"] <-     p_SD
...
m_P_trtA <- m_P_SoC 
m_P_trtA["H", "H"] <- (1 - p_HD) * (1 - p_HS_trtA)
m_P_trtA["H", "S"]    <- (1 - p_HD) *      p_HS_trtA

## Treatment B
# Store the same matrix of SoC in Trt B, only replace different parameters
m_P_trtB <- m_P_SoC
m_P_trtB["H", "H"] <- (1 - p_HD) * (1 - p_HS_trtB)
m_P_trtB["H", "S"]    <- (1 - p_HD) *      p_HS_trtB
