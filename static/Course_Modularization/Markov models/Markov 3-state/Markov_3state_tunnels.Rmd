...
```{r}
# Strategy names
v_names_str <- c("Standard of Care", "Treatment A", "Treatment B")  

# Markov model parameters
v_names_states  <- c("Healthy", "Sick", "Dead")  # state names
n_t  <- 60                                       # number of cycles
v_names_cycles  <- paste("cycle", 0:n_t)         # cycle names

# Transition probabilities
p_HD_min  <- 0.003                           # probability of dying when healthy at t = 0
p_HD_max  <- 0.01                            # probability of dying when healthy at t = n_t
p_HS_SoC  <- 0.05                            # probability of becoming sick when healthy, conditional on surviving, under SoC
...
c_trtB    <- 2000                            # cost of treatment B (per cycle)
...
v_p_HD    <- seq(p_HD_min, p_HD_max, length.out = n_t)  
...
m_M_SoC[1, ] <- m_M_trtA[1, ] <- m_M_trtB[1, ] <- c(1, rep(0, n_tunnel_size), 0)     
```
...
for(i in 1:(n_tunnel_size - 1)){ 
  a_P_SoC[v_Sick_tunnels[i], v_Sick_tunnels[i + 1], ] <- 1 - p_SD[i]
  a_P_SoC[v_Sick_tunnels[i], "Dead", ] <- p_SD[i]
}

a_P_SoC[v_Sick_tunnels[n_tunnel_size], v_Sick_tunnels[n_tunnel_size], ] <- 1 - p_SD[n_tunnel_size]
a_P_SoC[v_Sick_tunnels[n_tunnel_size], "Dead", ] <- p_SD[n_tunnel_size]
...
check_sum_of_transition_array(a_P_SoC,  n_states = n_states_tunnels, n_cycles = n_t, verbose = TRUE)
check_sum_of_transition_array(a_P_trtA, n_states = n_states_tunnels, n_cycles = n_t, verbose = TRUE)
check_sum_of_transition_array(a_P_trtB, n_states = n_states_tunnels, n_cycles = n_t, verbose = TRUE)
```
...
v_tc_trtA <- m_M_tunnels_trtA %*% c(c_H, c_S + c_trtA, c_D)  
v_tc_trtB <- m_M_tunnels_trtB %*% c(c_H, c_S + c_trtB, c_D)  
