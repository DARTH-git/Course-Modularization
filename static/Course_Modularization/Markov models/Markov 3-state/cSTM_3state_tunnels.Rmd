...
l_m_M_tunnels <- list (m_M_tunnels_SoC,
                       m_M_tunnels_trtA,
                       m_M_tunnels_trtB)
names(l_m_M_tunnels) <- v_names_str
...
for (i in 1:n_str) { # i <- 1
  ...
  v_qaly_str <- l_m_M_tunnels[[i]] %*% v_u_str # sum the utilities of all states for each cycle
  v_cost_str <- l_m_M_tunnels[[i]] %*% v_c_str # sum the costs of all states for each cycle
