...
decision_model <- function(l_params_all, verbose = FALSE) {
  with(as.list(l_params_all), {
    ...
    m_M_no_trt <- m_M_trt <- matrix(NA, 
                                    ...
                                    dimnames = list(paste("cycle", 0:n_t, sep = " "), v_names_states_td)) # Update!
    ...
    a_P_no_trt <- array(0,                                          # Create 3-D array
                       ...
                       dimnames = list(v_names_states_td, v_names_states_td, 0:(n_t-1)))  # name dimensions of the transition probability array
