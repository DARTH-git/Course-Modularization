...
v_r_mort_by_age <- lt_usa_2015$Total
names(v_r_mort_by_age) <- paste0("Age",lt_usa_2015$Age)
...
```{r}
## Initialize cohort trace for age-dependent (ad) cSTM for SoC
m_M_ad <- matrix(NA, 
...
m_M_ad_strA  <- m_M_ad # Strategy A
m_M_ad_strB  <- m_M_ad # Strategy B
m_M_ad_strAB <- m_M_ad # Strategy AB
```
...
a_A_strA <- a_A_strB <- a_A_strAB <- a_A
```
...
for(t in 1:n_cycles){
  ...
  # For strategy A
  m_M_ad_strA[t + 1, ]  <- m_M_ad_strA[t, ]   %*% a_P_strA[, , t]
  # For strategy B 
  m_M_ad_strB[t + 1, ]  <- m_M_ad_strB[t, ]   %*% a_P_strB[, , t]
  # For strategy AB
  m_M_ad_strAB[t + 1, ] <- m_M_ad_strAB[t, ]  %*% a_P_strAB[, , t]
  
  ## Fill in transition-dynamics array
  # For SoC
  a_A[, , t + 1]        <- m_M_ad[t, ]         *  a_P[, , t]
  # For strategy A 
  a_A_strA[, , t + 1]   <- m_M_ad_strA[t, ]    *  a_P_strA[, , t]
  # For strategy B
  a_A_strB[, , t + 1]   <- m_M_ad_strB[t, ]    *  a_P_strB[, , t]
  # For strategy AB
  a_A_strAB[, , t + 1]  <- m_M_ad_strAB[t, ]   *  a_P_strAB[, , t]
}
...
```{r Sick-Sicker-Trace-SimTimDep, fig.cap='Cohort trace of the simulation-time dependent cSTM under standard of care', message=FALSE, warning=FALSE, fig.pos="H", }
# create a plot of the data
plot_trace(l_m_M$`Standard of care`)
```

```{r Sick-Sicker-Trace-SimTimDep, fig.cap='Cohort trace by state of the simulation-time dependent cSTM under all strategies', message=FALSE, warning=FALSE, fig.pos="H"}
...
```{r Sick-Sicker-Trace-SimTimDep, fig.cap='Overall survival of the simulation-time dependent cSTM.', message=FALSE, warning=FALSE, fig.pos="H"}
survival_plot <- plot_surv(l_m_M, v_names_death_states = "D") + theme(legend.position = "bottom")
survival_plot
```
...
```{r Sick-Sicker-DisPrev-SimTimDep, fig.cap='Disease prevalence of the simulation-time dependent cSTM.', message=FALSE, warning=FALSE, fig.pos="H"}
...
prevalence_S2_plot <- plot_prevalence(l_m_M, 
                                      ...
                                      v_names_dead_states = "D")  +
                                      theme(legend.position = "")
grid.arrange(prevalence_S1_plot, prevalence_S2_plot)
```

## 06.4 Proportion of sick in S1 state

```{r Sick-Sicker-PropS2-SimTimDep, fig.cap='Proportion of S2 among S1 and S2 of the simulation-time dependent cSTM.', message=FALSE, warning=FALSE, fig.pos="H"}
...
for (i in 1:n_str) {
  ...
  # Note: all parameters are annual in our example. In case your own case example is different make sure you correctly apply conversion to generate annual outcomes.
...
## 07.5 Plot efficient frontier

```{r Sick-Sicker-CEA, fig.cap='Cost-effectiveness efficient frontier for the simulation-time dependent Sick-Sicker model.', message=FALSE, warning=FALSE, fig.pos="H"}
