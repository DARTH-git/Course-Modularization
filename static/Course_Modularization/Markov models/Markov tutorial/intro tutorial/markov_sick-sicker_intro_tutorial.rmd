...
pacman::p_load("dplyr", "data.table", "devtools", "scales", "ellipse", "ggplot2", 
               ...
               "stringr", "gridExtra", "diagram", "dampack", "boot", "doParellel")                                               
...
```{r Sick-Sicker-Trace-TimeHom, fig.cap='Cohort trace of the time-independent cSTM under standard of care', message=FALSE, warning=FALSE, fig.pos="H"}
...
```{r Sick-Sicker-OS-TimeHom, fig.cap='Overall survival of the time-independent cSTM under standard of care.', message=FALSE, warning=FALSE, fig.pos="H"}
...
```{r Sick-Sicker-DisPrev-TimeHom, fig.cap='Disease prevalence of the time-independent cSTM under standard of care.', message=FALSE, warning=FALSE, fig.pos="H"}
...
```{r Sick-Sicker-PropS1-TimeHom, fig.cap='Proportion of sick in S1 state of the time-independent cSTM under standard of care.', message=FALSE, warning=FALSE, fig.pos="H"}
...
#*possible outcomes of the OWSA depend on the outcomes the function gives 
```
...
owsa_tornado(owsa = owsa_nmb, txtsize = 8, return = "plot") 
```
...
n_time_init_psa_series <- Sys.time()  # store the system time at start
for(i in 1:n_sim){
...
n_time_end_psa_series   <- Sys.time() # store the system time at end 
n_time_total_psa_series <- n_time_end_psa_series - n_time_init_psa_series # calculate the PSA duration 
...
# CEA Scatter plot
plot(l_psa, txtsize = 14, ellipse = TRUE) +
...
CEAF: indicating which strategy is economically preferred at different threshold values for cost-effectiveness
NOTE: The CEAF is informative in addition to the CEAC, because the strategy that is most likely to be cost-effective should not be conflated with the strategy that is optimal in expectation in the decision-making process
...
The expected loss is the quantification of the foregone benefits when choosing a sub-optimal strategy given current evidence.
