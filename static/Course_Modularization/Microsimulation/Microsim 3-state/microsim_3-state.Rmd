...
# Model structure
v_names_states <- c("H", "S", "D")  # vector with state names, Healthy (H), Sick (S), Dead (D)
n_states    <- length(v_names_states)  # number of states
n_cycles    <- 60                      # number of cycles
n_i         <- 10000                   # number of individuals
d_e         <- d_c <- 0.03             # equal discount of costs and QALYs 
...
Effs <- function (M_t) {
  ...
  q_t[M_t == "S"] <- u_S  # QALYs accrued by being Sick this cycle
