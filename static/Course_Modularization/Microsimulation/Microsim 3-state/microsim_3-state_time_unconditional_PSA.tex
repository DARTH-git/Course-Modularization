...
\NormalTok{n_s   <-}\StringTok{ }\KeywordTok{length}\NormalTok{(v_n)                           }\CommentTok{# number of states}
...
\NormalTok{df_X  <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{ID =} \DecValTok{1}\OperatorTok{:}\NormalTok{n_i, }\DataTypeTok{Sex =}\NormalTok{ v_sex)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  df_out_temp <-}\StringTok{ }\KeywordTok{calculate_ce_out}\NormalTok{(df_psa_input[i, ], }\DataTypeTok{n_wtp =} \DecValTok{10000}\NormalTok{)}
...
\NormalTok{  df_e[i, ] <-}\StringTok{ }\NormalTok{df_out_temp}\OperatorTok{$}\NormalTok{Effect}
  ...
  \ControlFlowTok{if}\NormalTok{(i}\OperatorTok{/}\NormalTok{(n_sim}\OperatorTok{/}\DecValTok{10}\NormalTok{) }\OperatorTok{==}\StringTok{ }\KeywordTok{round}\NormalTok{(i}\OperatorTok{/}\NormalTok{(n_sim}\OperatorTok{/}\DecValTok{10}\NormalTok{), }\DecValTok{0}\NormalTok{)) \{ }\CommentTok{# display progress every 10%}
    \KeywordTok{cat}\NormalTok{(}\StringTok{'}\CharTok{\textbackslash{}r}\StringTok{'}\NormalTok{, }\KeywordTok{paste}\NormalTok{(}\StringTok{'            '}\NormalTok{, }\StringTok{'Overall progress: '}\NormalTok{, i}\OperatorTok{/}\NormalTok{n_sim }\OperatorTok{*}\StringTok{ }\DecValTok{100}\NormalTok{, }\StringTok{"% done"}\NormalTok{, }
...
\subsection{08.3 Cost Effectiveness
Analysis}\label{cost-effectiveness-analysis}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# make an PSA object using dampack}
\NormalTok{out_psa  <-}\StringTok{ }\KeywordTok{make_psa_obj}\NormalTok{(df_c, df_e, df_psa_input, }\DataTypeTok{strategies =} \OtherTok{NULL}\NormalTok{, }\StringTok{"$"}\NormalTok{)}
\CommentTok{# Total cost and effectiveness}
\NormalTok{results  <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\StringTok{"Total Cost"}\NormalTok{ =}\StringTok{ }\KeywordTok{mean}\NormalTok{(out_psa}\OperatorTok{$}\NormalTok{cost[, }\DecValTok{1}\NormalTok{]), }
                       \StringTok{"Total QALYs"}\NormalTok{ =}\StringTok{ }\KeywordTok{mean}\NormalTok{(out_psa}\OperatorTok{$}\NormalTok{effectiveness[, }\DecValTok{1}\NormalTok{]))}
\NormalTok{results}

\CommentTok{# Cost-Effectiveness Scatter plot }
\KeywordTok{plot}\NormalTok{(out_psa)}
\end{Highlighting}
\end{Shaded}
