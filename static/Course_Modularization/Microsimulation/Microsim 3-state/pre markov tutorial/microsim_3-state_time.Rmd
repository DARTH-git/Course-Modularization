...
df_p_HD     <- data.frame(Sex = c("Female", "Male"), p_HD = c(p_HD_female, p_HD_male))
...
df_X  <- data.frame(ID = 1:n_i, Sex = v_sex, n_ts = v_Ts_init, M_init = v_M_init)
# NOTE: we use n_ts for the number of times being sick, we start the data frame with the initial "history" of time being sick by saving v_Ts_init. However, during the simulation this value is updated and therefore called numer of times being sick.
...
Probs <- function(M_t, df_X) { 
  ...
  m_p_t["Sick",    M_t == "Sick"]    <-  1 - p_SD[df_X$n_ts]
  m_p_t["Dead",    M_t == "Sick"]    <-      p_SD[df_X$n_ts]     
...
MicroSim <- function(n_i, df_X, seed = 1) {
  ...
  for (t in 1:n_t) {
    ...
    # once someone is not "Sick" anymore, we reset n_ts (set back to zero)
    df_X$n_ts <- if_else(m_M[, t + 1] == "Sick", df_X$n_ts + 1, 0) 
...
plot_trace_microsim(outcomes$m_M)  # health state trace
```
