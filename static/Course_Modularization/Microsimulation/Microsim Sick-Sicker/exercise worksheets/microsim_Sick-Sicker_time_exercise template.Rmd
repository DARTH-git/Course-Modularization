...
source("../DARTH_affiliations_script.R")
...
#v_Ts_init <- [YOUR TURN]  # everyone begins in the healthy state (in this example)
```
...
#df_X <- df_X <- data.frame(ID = 1:n_i, x = v_x, Age = v_age0, n_cycles_s = v_Ts_init, M_init = v_M_init) 
...
(2) The probability from Sick to dead (`p_S1D`) depends on the number of cycles you have been in sick (`n_cycles_s`)
...
Remember treatment is given to both individuals in Sick and Sicker but only improved the quality of life of those in Sick. This utility has to be adjusted for the adjust for individual effect modifier. 
```{r}
# Your turn
