...
\CommentTok{#p_S1D    <- rep(0.0149, n_t)          # probability to die in S1 by cycle }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{start.time <-}\KeywordTok{proc.time}\NormalTok{()}
\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{n_sim)\{}
\NormalTok{  df_ce_psa <-}\StringTok{ }\KeywordTok{calculate_ce_out}\NormalTok{(df_psa_input[i, ])}
\NormalTok{   df_c[i, ] <-}\StringTok{ }\NormalTok{df_ce_psa}\OperatorTok{$}\NormalTok{Cost   }\CommentTok{# take the cost from the psa run and store in df_c}
\NormalTok{   df_e[i, ] <-}\StringTok{ }\NormalTok{df_ce_psa}\OperatorTok{$}\NormalTok{Effect }\CommentTok{# take the cost from the psa run in store in a df_e}
  \CommentTok{# Display simulation progress}
...
\NormalTok{  \}}
\NormalTok{\}}
\NormalTok{elapsed.time <-}\KeywordTok{proc.time}\NormalTok{()}\OperatorTok{-}\NormalTok{start.time}

\CommentTok{### Creae PSA object for dampack}
...
\begin{Highlighting}[]
\CommentTok{# use dampack to calculate the ICER}
\KeywordTok{calculate_icers}\NormalTok{(}\DataTypeTok{cost       =}\NormalTok{ df_ce_psa}\OperatorTok{$}\NormalTok{Cost,}
                \DataTypeTok{effect     =}\NormalTok{ df_ce_psa}\OperatorTok{$}\NormalTok{Effect,}
                \DataTypeTok{strategies =}\NormalTok{ df_ce_psa}\OperatorTok{$}\NormalTok{Strategy)}
\end{Highlighting}
\end{Shaded}
