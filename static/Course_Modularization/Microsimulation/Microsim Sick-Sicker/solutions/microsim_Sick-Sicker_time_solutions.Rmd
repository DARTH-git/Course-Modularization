...
```{r}
MicroSim <- function(n_i, df_X, Trt = "Standard of care", seed = 1, cl = 1) {
  # Arguments:  
  ...
  m_M <- m_C <- m_E <-  matrix(nrow = n_i, ncol = n_cycles + 1, 
    ...
    m_E[, 1] <- Effs (m_M[, 1], df_X, Trt = Trt, cl = cl) # calculate QALYs per individual during cycle 0
    ...
    for (t in 1:n_cycles) {
      ...
      m_E[, t + 1]  <- Effs(m_M[, t + 1], df_X, Trt, cl = cl)    
...
outcomes_SoC   <- MicroSim(n_i = n_i, df_X = df_X, Trt = "Standard of care", seed = 1, cl = cycle_length)
outcomes_trtAB <- MicroSim(n_i = n_i, df_X = df_X, Trt = "Strategy AB",      seed = 1, cl = cycle_length)
```
