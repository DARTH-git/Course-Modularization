...
1.  Division of Public Administration, Center for Research and Teaching
    in Economics (CIDE), Aguascalientes, Mexico
...
-   Alarid-Escudero F, Krijkamp EM, Enns EA, Yang A, Hunink MGM
    ...
    Example. Medical Decision Making, 2022 (In press): 1-21.
...
-   Krijkamp EM, Alarid-Escudero F, Enns E, Pechlivanoglou P, Hunink MM,
    ...
    Model Dynamics. Med Decis Mak. 2020;40(2):242-248.
...
### Transition intenisty rates
r_HS_SoC  <- 0.05  # rate of becoming sick when healthy, conditional on surviving, under standard of care
r_HS_trtA <- 0.04  # rate of becoming sick when healthy, conditional on surviving, under treatment A
r_HS_trtB <- 0.02  # rate of becoming sick when healthy, conditional on surviving, under treatment B
...
v_r_HD    <- r_base * rr_annual ^ (1:n_cycles)   
...
check_transition_probability(a_P_SoC,  verbose = FALSE)
check_transition_probability(a_P_trtA, verbose = FALSE)
check_transition_probability(a_P_trtB, verbose = FALSE)
# Check that all rows sum to 1
check_sum_of_transition_array(a_P_SoC,  n_states = n_states, n_cycles = n_cycles, verbose = FALSE)
check_sum_of_transition_array(a_P_trtA, n_states = n_states, n_cycles = n_cycles, verbose = FALSE)
check_sum_of_transition_array(a_P_trtB, n_states = n_states, n_cycles = n_cycles, verbose = FALSE)
```
...
l_params_all <- list(
 ...
 rr_annual  = 1.1,  # annual increase of mortality rate
  ...
  u_D       = 0,     # utility when dead
  # Discount rates
  d_e       = 0.015,  # discount rate per cycle equal discount of costs and QALYs by 3%
...
```{r}
#* Extra material, renaming the x-axis

owsa_nmb <- owsa_nmb %>%
  mutate(parameter = recode(parameter,
                            "c_trtA" = "Cost treatment A",
                            "c_trtB" = "Cost treatment B",
                            "c_S" = "Cost Sick"))
