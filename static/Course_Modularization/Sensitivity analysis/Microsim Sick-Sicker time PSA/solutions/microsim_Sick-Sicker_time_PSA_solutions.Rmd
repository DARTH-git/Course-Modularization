...
subtitle: 'Includes individual characteristics: age, age dependent mortality, individual treatment effect modifier, state-residency for the sick (S1) state, increasing change of death in the first 6 years of sickness'
...
v_dwe <- v_dwc <- 1 / ((1 + d_r) ^ (0:n_cycles))    # discount weight 
v_names_str <- c("Soc", "treatmentAB")  # strategy names
...
p_mort   <- read.csv("solutions/mortProb_age.csv")
# load age distribution
dist_Age <- read.csv("solutions/MyPopulation-AgeDistribution.csv")
...
u_trtAB <- 0.95             # utility when sick and being treated
```
...
outcomes_SoC <- MicroSim(n_i, df_X, Trt = "Standard of care", seed = 1)
outcomes_trtAB    <- MicroSim(n_i, df_X, Trt = "Strategy AB",  seed = 1)
...
```{r}
source("solutions/Function_Microsim_Sick-Sicker_time.R")
