...
We use the `gen_psa_df` function to generate as PSA dataset with parameter values for each PSA iteration. the parameter values are sampled from their corresponding distribution as is specified in the input data. 

```{r}
m_psa_input <- gen_psa_df(df_param = df_param, n_iter = 5) # test run with 5 iterations 

# Note the output of gen_psa_df is a matrix 
```

# Visually inspect the sampled results 

```{r}
m_psa_input <- gen_psa_df(df_param = df_param, n_iter = 10000) # test run with 5 iterations 
# Adjust the numbers of which parameters you like to see here

df_psa_input <- as.data.frame(m_psa_input)

# Note the 
ggplot(reshape2::melt(df_psa_input, variable.name =  "Parameter"), aes(x = value)) +
...
# Test for code with df from 3 state model

```{r, eval = FALSE}
load("../Markov 3-state SA/df_psa_input.rda") # df_psa_input
# df_psa_input is form the 3-state model

df_PSA_test1 <- df_psa_input
m_psa_input <- gen_psa_df(df_param = df_param, n_iter = 1e6, seed = 071818)
...
# However, with large numbers we can compare if the mean values are close
