...
pacman::p_load("dplyr", "devtools", "scales", "ellipse", "ggplot2", "lazyeval", 
               "igraph", "truncnorm", "ggraph", "reshape2", "knitr", "stringr", 
               "diagram", "dampack")                                               
...
d_e      <- 0.03                     # discount rate per cycle for costs by 3%
d_c      <- 0.03                     # discount rate per cycle for QALYs by 3%
...
```{r STD-Sick-Sicker, fig.cap="State-transition diagram of the time-independent Sick-Sicker cohort state-transition model.", fig.pos="H"}
m_P_diag <- matrix(0, 
...
```{r}
# create the markov trace matrix M capturing the proportion of the cohort in
# each state at each cycle
...
m_M_notrt[1, ] <- c(1, 0, 0, 0) # initiate first cycle of cohort trace
m_M_trt[1, ]   <- c(1, 0, 0, 0) # initiate first cycle of cohort trace
```
...
m_P_notrt <- m_P_trt <- matrix(0,
                               ...
                               dimnames = list(v_names_states, 
                                               v_names_states)) # name the columns and rows of the matrix
m_P_notrt
```
...
check_sum_of_transition_array(m_P_notrt, n_states = n_states, verbose = TRUE)
...
```{r Sick-Sicker-Trace-TimeHom, fig.cap='Cohort trace of the time-independent cSTM under no treatment.', message=FALSE, warning=FALSE, fig.pos="H"}
...
legend("topright", v_names_states, 
       col = 1:n_states, lty = 1:n_states, bty = "n") 
```

## 06.2 Overall Survival (OS)

```{r Sick-Sicker-OS-TimeHom, fig.cap='Overall survival of the time-independent cSTM under no treatment.', message=FALSE, warning=FALSE, fig.pos="H"}
...
plot(0:n_cycles, v_os_notrt, type = 'l', 
     ylim = c(0, 1),
...
```{r Sick-Sicker-DisPrev-TimeHom, fig.cap='Disease prevalence of the time-independent cSTM under no treatment.', message=FALSE, warning=FALSE, fig.pos="H"}
...
```{r Sick-Sicker-PropS1-TimeHom, fig.cap='Proportion of sick in S1 state of the time-independent cSTM under no treatment.', message=FALSE, warning=FALSE, fig.pos="H"}
...
# data.frame with discounted costs and effectiveness
...
```{r Sick-Sicker-CEA, fig.cap='Cost-effectiveness efficient frontier for the time-independent Sick-Sicker model.', message=FALSE, warning=FALSE, fig.pos="H"}
...
```{r Sick-Sicker-OWSA, fig.cap='One-way sensitivity analysis for the time-independent Sick-Sicker model.', message=FALSE, warning=FALSE, fig.pos="H"}
...
```{r Sick-Sicker-OptStrOWSA, fig.cap='Optimal strategy with one-way sensitivity analysis for the time-independent Sick-Sicker model.', message=FALSE, warning=FALSE, fig.pos="H"}
owsa_opt_strat(owsa = owsa_nmb, txtsize = 10)
```

### 08.3.3 Tornado plot

```{r Sick-Sicker-Tornado, fig.cap='Tornado plot for the time-independent Sick-Sicker model.', message=FALSE, warning=FALSE, fig.pos="H"}
...
```{r Sick-Sicker-TWSA, fig.cap='Two-way sensitivity analysis for the time-independent Sick-Sicker model.', message=FALSE, warning=FALSE, fig.pos="H"}
plot(twsa_nmb)
```
...
```{r Sick-Sicker-ParamsHist, fig.cap='Histogram of parameters distributions for the time-independent Sick-Sicker model.', message=FALSE, warning=FALSE, fig.pos="H"}
...
```{r CE-scatter, echo=FALSE, fig.cap='Cost-effectiveness scatter plot.', message=FALSE, warning=FALSE, fig.pos="H"} 
# CEA Scatter plot
...
```{r Sick-Sicker-CEA-PSA, fig.cap='Cost-effectiveness efficient frontier from probabilistic outputs for the time-independent Sick-Sicker model.', message=FALSE, warning=FALSE, fig.pos="H"}
plot(df_cea_psa)
```
...
```{r ELC, echo=FALSE, fig.cap='Expected loss curves (ELCs).', message=FALSE, warning=FALSE, fig.pos="H"}
...
```{r EVPI, echo=FALSE, fig.cap='Expected value of perfect information (EVPI).', message=FALSE, warning=FALSE, fig.pos="H"}
