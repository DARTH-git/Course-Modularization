...
1. Add a scatter plot with eruptions on the y-axis and waiting time on the x-axis below the histogram in the App. 

2. Modify the App to allow the user to choose the color of the scatter plot. The user can choose from red, blue, green, orange and purple. 

**EXERCISE 2** Follow the steps below to build a Shiny App that hosts our simple 3-state microsimulation model with modifiable input parameters.

- Make sure to download all relevant files including `Function_MicroSim_3state.R`, `Functions_Input.R`, and `ShinyApp.R`.

- Open up `ShinyApp.R`.

- `sliderInput()` creates a slider with minimum, maximum and initial values.

- Create one such slider input for the parameter `c_S` (cost of being in the sick state) with minimum value = 100, maximum value = 1000 and initial value = 500. Make sure to add an appropriate label and position the slider below all previous sliders. 

- Create one such slider input for the parameter `u_S` (cost of being in the sick state) with minimum value = 0, maximum value = 1 and initial value = 0.85 Make sure to add an appropriate label and position the slider below the slider for `c_S`. 
