...
library(gems)
source("traceDES2.R")
trace.DES2(DES_data$t[1:10,], times=times)
```
...
best.fit_HS <- fit_HS$models$Gamma
best.fit_HD <- fit_HD$models$`Weibull (AFT)`
best.fit_SD <- fit_SD$models$`log-Normal`
```
