...
# Note: no information here about the transition from HS and from HD
...
# 03.1 Input Survival analysis component
Survival analysis is a branch of statistics that deals with the analysis of time-to-event data. The goal is to model and predict the time until an event of interest occurs, such as death or disease progression.  
```{r}
...
# Fit independent models for each transition 
```{r}
...
fitkm_HD <- survfit(Surv(time, status) ~ 1, data = data_HD) # estimate Kaplan Meier curve
summary(fitkm_HD, t = v_times) # Summarize the fitted curve 
plot(fitkm_HD)  # plot the curve 
...
survfit2(Surv(time, status) ~ 1, data = data_HS) %>%
    ggsurvfit() +
     ...
     add_risktable() +
     scale_ggsurvfit() +
     ggtitle("KM-curve Healthy to Sick")
...
survfit2(Surv(time, status) ~ 1, data = data_HD) %>%
    ggsurvfit() +
     ...
     add_risktable() +
     scale_ggsurvfit() +
  ggtitle("KM-curve Healthy to Dead")
...
fitsurvHE_HS <- fit.models(Surv(time, status) ~ 1, data = data_HS, distr = v_survival_distribution ) # all values 
...
names(v_AIC_HS) <- names(v_BIC_HS) <- v_survival_distribution
...
fitsurvHE_HD <- fit.models(Surv(time, status) ~ 1, data = data_HD, distr = v_survival_distribution ) # all values 
...
names(v_AIC_HD) <- names(v_BIC_HD) <- v_survival_distribution
