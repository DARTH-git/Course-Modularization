...
data_HS <- l_data_survival$data_HS # Substract the data for the transition from H to S
head(data_HS) # patient ID, transition, time and status (1 = event, 0 = censored)

data_HD  <- l_data_survival$data_HD  # Substract the data for the transition from H to D
...
plot(fitflex_HS) # plot the survival curve, #default is that the estimated is plotted against the observed. 
...
fitsurvHE_HS <- fit.models(Surv(time, status)~1, data = data_HS, distr = c("exponential", "gamma", "weibull") ) # all values 
...
p_HS    <- trans_prob(surv_prob(best_HS, times = v_times)) 
p_HD    <- trans_prob(surv_prob(best_HD,  times = v_times))
