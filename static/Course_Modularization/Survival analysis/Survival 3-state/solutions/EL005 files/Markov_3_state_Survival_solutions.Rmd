...
# To Do: Rename to healthy - sick - dead to make it in line with the 3-state model we normally have
...
v_names_states    <- c("S1", "S2", "D")   # the model states names, Sick (S1), Sicker (S2) and Dead

# Model structure 
n_cycles <- 30           # time horizon, 30 cycles
c_l      <- 1            # cycle length, 1 year
d_r      <- 0.015        # discount rate of 1.5% per cycle #@Petros: why do we have only 1.5% per year? And not the d_e and d_c as we normally have?
#d_e 
#d_c 
p_S2D    <- 0.2          # probability of dying in sicker state #@Petros: annual prob?

# initial cohort distribution (everyone allocated to the "S1" state)
v_init <- c("S1" = 1,
            "S2" = 0,
            "D"  = 0)  

# Cost inputs
# To do: update according to 3-state model (same utilities)
...
```{r}
# Load the Sicker data, structure is a list
# to do: adjust the file names (S1D etc) and resave a an .RDS file
l_data_survival <- readRDS("Data_Survival.RDS")


data_S1S2 <- l_data_survival$data_S1S2 # Substract the data for the transition from S1 to S2
head(data_S1S2) # patient ID, transition, time and status (1 = event, 0 = censored)

data_S1D  <- l_data_survival$data_S1D  # Substract the data for the transition from S1 to S2
head(data_S1D) # patient ID, transition, time and status (1 = event, 0 = censored)
...
fitkm_S1S2 <- survfit(Surv(time, status)~1, data = data_S1S2) # estimate Kaplan Meier curve
# 1, indicate we are not having covariates, we only use the time and status
summary(fitkm_S1S2, t = v_times) # Summarize the fitted curve 
plot(fitkm_S1S2) # plot the curve 

# fit parametric models using flexsurv
fitflex_S1S2 <- flexsurvreg(Surv(time, status)~1, data = data_S1S2 , dist = "weibull")
fitflex_S1S2 # print the model parameters 
summary(fitflex_S1S2, t = v_times) # summarize the survival 
plot(fitflex_S1S2) # plot the survival curve, #default is that the estimated is plotted against the observed. 
#@Petros: why is this plot added the the previous KM-curve we had? 

# flexsurvreg: estimates the curves 
# fit.model: is a wrapper to run the function many times. 
# we have a function that does the same survival analysis curve fitting for the different specified distributions 

# using the survHE package 
# @Petros: why are we moving to survHE, and do not to exponential, gamma and Weibull with the flexsurvreg function?
fitsurvHE_S1S2 <- fit.models(Surv(time, status)~1, data = data_S1S2, distr = c("exponential", "gamma", "weibull") ) # all values 

plot(fitsurvHE_S1S2)

# add code to extra the AIC from the just fitted function
# provide some code that 

# to do Survival:add a describtion that this does the same, just in a way we as modellers find it usefull
# defaul is all the distribution 
fit_S1S2  <- fit.fun(time = "time", status = "status", data = data_S1S2, times = v_times)
fit_S1D   <- fit.fun(time = "time", status = "status", data = data_S1D,  times = v_times)
...
# trans_prob is a function from darthtools and calculated the transition probabilities using survival probabilities

# first have the 
surv_prob(best_S1S2, times = v_times) # survival probabilities for the best fitting model
surv_prob(best_S1D, times = v_times) # survival probabilities for the best fitting model
# if you would like to adjust for the time , create a new vector or times based on the new cycle length 


# Next, use this survival probability to estimate the transition probability (assuming still all annual)
p_S1S2    <- trans_prob(surv_prob(best_S1S2, times = v_times)) 
p_S1D     <- trans_prob(surv_prob(best_S1D,  times = v_times))
