...
p_load( "dplyr", "devtools", "gems", "flexsurv", "survminer", "survHE", "ggplot2", "msm", "igraph", "mstate", "reshape2", "knitr", "diagram", "abind", "DES")    
...
```{r}
m_P_diag <- matrix(NA, n_states, n_states, dimnames = list(v_n,v_n))
...
```
The following chink generates a hypothetical dataset from a Randomized Controlled trial. The two outcomes are Sickness(or Progression) and Death. We generate four data structures:
...
-status includes information regarding whether the event occured or not
...
best.fit_HS <- fit_HS$models$`log-Normal`
best.fit_HD <- fit_HD$models$`Gompertz`
best.fit_SD <- fit_SD$models$`Spline`
```
...
p_HS <- trans_prob(surv_prob(best.fit_HS))
p_HD <- trans_prob(surv_prob(best.fit_HD))
p_SD <- trans_prob(surv_prob(best.fit_SD))
...
Probs <- function(M_t, v_Ts, t) { 
  ...
  m_p_t[, M_t == "Healthy"] <- rbind( (1 - p_HD[t]) * (1 - p_HS[t]),
                                      (1 - p_HD[t]) *      p_HS[t],
                                                           p_HD[t])    
...
# matlines(times, m_M_PSM$trace,   col = 4, lty = 1)
