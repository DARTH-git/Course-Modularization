...
p_load("here", "dplyr", "devtools", "gems", "flexsurv", "flexsurvcure", "survHE", "survminer", "ggplot2", "msm", "igraph", "mstate", "reshape2", "knitr", "abind", "dampack", "data.table")  
...
```{r}
v_names_states <- c("1st Relapse", "2nd Relapse", "Dead")  # state names
...
# Create IPD and KM data for the OS curves
digitise(surv_inp   = "PFS_BSC_correct.txt", 
         nrisk_inp  = "PFS_BSCatrisk.txt", 
         km_output  = "BSC_KMdata_PFS.txt", 
         ipd_output = "BSC_IPDdata_PFS.txt")

# Create IPD and KM data for the PFS curves (there have been 99 events in the cohort)
digitise(surv_inp   = "PFS_Blina_correct.txt", 
         nrisk_inp  = "PFS_Blinaatrisk.txt", 
         km_output  = "Blina_KMdata_PFS.txt", 
         ipd_output = "Blina_IPDdata_PFS.txt")


# Create IPD and KM data for the OS curves
digitise(surv_inp   = "OS_BSC_correct.txt", 
         nrisk_inp  = "OS_BSCatrisk.txt", 
         km_output  = "BSC_KMdata_OS.txt", 
         ipd_output = "BSC_IPDdata_OS.txt")

# Create IPD and KM data for the PFS curves (there have been 99 events in the cohort)
digitise(surv_inp   = "OS_Blina_correct.txt", 
         nrisk_inp  = "OS_Blinaatrisk.txt", 
         km_output  = "Blina_KMdata_OS.txt", 
         ipd_output = "Blina_IPDdata_OS.txt")
...
IPD_OS_BSC  <- make.ipd(ipd_files = c("BSC_IPDdata_OS.txt"), ctr = 1, 
                    var.labs  = c("time","event","arm"))
IPD_PFS_BSC <- make.ipd(ipd_files = c("BSC_IPDdata_PFS.txt"), ctr = 1,
                    var.labs  = c("time","event","arm"))

# Link the IPD files across the two arms of the trial for OS and PFS
IPD_OS_Blina  <- make.ipd(ipd_files = c("Blina_IPDdata_OS.txt"), ctr = 1, 
                    var.labs  = c("time","event","arm"))
IPD_PFS_Blina <- make.ipd(ipd_files = c("Blina_IPDdata_PFS.txt"), ctr = 1,
                    var.labs  = c("time","event","arm"))


PFS <- data.frame 
IPD_OS$time[IPD_OS$time ==0] = 0.01
IPD_PFS$time[IPD_PFS$time ==0] = 0.01
```
...
m_M_PSM <- partsurv(pfs_survHE = fit_PFS, os_survHE  = fit_OS,
                    choose_PFS = choose_PFS,  choose_OS  = choose_OS, 
                    time = times, v_names_states = v_names_states)
