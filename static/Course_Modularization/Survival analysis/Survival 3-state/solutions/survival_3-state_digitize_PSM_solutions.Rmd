...
```{r}
time_interval <- seq(0, 24, by = 3)
at_risk <- c(205, 172, 156, 143, 137, 120, 111, 101, 97)
surv_data <- read.csv("PFS-nivo+ipiCM227.csv", header = T)
surv_data$PFS.survival.rate... <- surv_data$PFS.survival.rate... / 100
surv_data <- data.frame(k = 1:nrow(surv_data),
                        Time = surv_data$Months,
                        Sk = surv_data$PFS.survival.rate...)
write.table(surv_data, file = "SurvivalUMN.txt", row.names = F)
surv_data <- read.table("SurvivalUMN.txt", header = T)
surv_times <- surv_data$Months

n_risk_table <- create_at_risk_table(survival_time = surv_times[1:83],
                                     Years = time_interval,
                                     AtRisk = at_risk)
write.table(n_risk_table, "RiskTableUMN.txt", row.names = F)
```
...
digitise(surv_inp   = "SurvivalUMN.txt", 
         nrisk_inp  = "RiskTableUMN.txt", 
         km_output  = "KMdata_UMN.txt", 
         ipd_output = "IPDdata_UMN.txt")
