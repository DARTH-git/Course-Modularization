...
data_S1D  <- data_long[data_long$from =="S1" & data_long$to =="D", ]
data_S1S2 <- data_long[data_long$from =="S1" & data_long$to =="S2",]
...
# Extract the transition probabilities from the fitted survival models
p_S1S2 <- trans_prob(fit_S1S2, choose_dist ="log-Logistic", times=times)$t.p
p_S1D  <- trans_prob(fit_S1D,  choose_dist ="Exponential", times=times)$t.p
```
