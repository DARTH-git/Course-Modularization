...
p_load("here", "dplyr", "devtools", "gems", "flexsurv", "survHE", "ggplot2", "msm", "igraph", "mstate", "reshape2", "knitr", "abind", "DES")  
...
n_s   <- length(v_names_states)    # the number of health states
...
best.fit_S1H  <- fit_S1H$ models[["Royston-Parmar"]]
best.fit_S1S2 <- fit_S1S2$models[["log-Logistic"]]
best.fit_S1D  <- fit_S1D$ models[["Exponential"]]
best.fit_S2D  <- fit_S2D$ models[["Weibull (AFT)"]]
...
plot_trace_microsim(outcomes_no_trt$m_M, v_names_states = v_names_states)  # health state trace
...
plot_trace_microsim(outcomes_trt$m_M, v_names_states = v_names_states)     # health state trace
```
