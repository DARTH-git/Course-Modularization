...
best.fit_S1H  <- fit_S1H$ models[[which.min(fit_S1H$AIC)]]
best.fit_S1S2 <- fit_S1S2$models[[which.min(fit_S1S2$AIC)]]
best.fit_S1D  <- fit_S1D$ models[[which.min(fit_S1D$AIC)]]
best.fit_S2D  <- fit_S2D$ models[["Weibull (AFT)"]]

# Extract transition probabilities from the best fitting models
p_S1H  <- surv_prob(best.fit_S1H)
p_S1S2 <- surv_prob(best.fit_S1S2)
p_S1D  <- surv_prob(best.fit_S1D)
p_S2D  <- surv_prob(best.fit_S2D)
```
